...
export default defineConfig(({ mode }: ConfigEnv): UserConfig => {
    ...
    return {
        ...
        plugins: [
            ...
            // ViteCompression({
            //     threshold: 1024 * 20, // 只有大小大于20kb的文件会被压缩
            //     ext: '.gz', // 压缩包后缀
            //     algorithm: 'gzip', // 压缩算法
            // }),
            // Vue模板文件编译插件
            ...
            AutoImport({
                ...
                resolvers: [ElementPlusResolver({ importStyle: 'css' }), IconsResolver()],
        ...
        build: {
            ...
            rollupOptions: {
                ...
                output: {
                    experimentalMinChunkSize: 20 * 1024, //单位: 字节(B)，这里是 20KB
                    manualChunks: (id: string) => {
                        ...
                        if (id.includes('src/views/about')) { 
                            return 'about' // 这样就可以像webpack一样打包了
                        }
                        // return 'index'
                    },
